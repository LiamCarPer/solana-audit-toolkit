...
**Date of run:** 2026-08-08 Â· **Tool version:** `sat` v0.1.0 (`cargo run --quiet --bin sat -- version`)

This benchmark runs `sat analyze src` against real, deployed Solana programs with
documented audit/hack histories â€” code the tool has never seen â€” and classifies
...
cross-instruction flows â€” those are documented limitations, not failures.
...
| Programs excluded (documented) | 2 (Mango v3, solana-program-library â€” not Anchor) |
| Actionable findings | 8 (5 LOW + 3 HIGH) + 6 informational |
| Findings matching a real exploit | **0 / 8** |
| Root-cause recall on Cashio | **0 / 1** (NOT-DETECTED â€” semantic class, documented) |
...
- Full outputs are committed in [`bench/`](../bench/) (`*.out` files) for
  reproducibility; SARIF export verified (`--format sarif` â†’ `sat-results.sarif`).
...
## Target 1 â€” Cashio, vulnerable commit (`a51c3c59`, 2022-03-10, pre-hack)

**Ground truth (rekt.news, verified source):** ~$48M lost via an "infinite mint".
Root cause: incomplete collateral validation â€” the LP-token deposit path via
`saber_swap.arrow` never validated the `.mint` field, so the attacker built a fake
root contract (a **false bank** â€” `new_bank` is permissionless) and a chain of fake
...
pool_mint` â€” all attacker-supplied; nothing references an approved registry.
...
| `NewBank::admin` missing signer | SAT001 | LOW | **FP** / HARDENING | permissionless `new_bank` by design â€” but this is the **exact instruction the exploit used as its "fake root contract"** |
| `PrintCash::issue_authority` missing signer | SAT001 | LOW | **FP** / HARDENING | validated in `Validate` impl (`assert_keys_eq!(â€¦, ISSUE_AUTHORITY_ADDRESS)`) + PDA signer seeds |
| `BurnCash::withdraw_authority` missing signer | SAT001 | LOW | **FP** / HARDENING | same pattern |
| Root cause (unvalidated `.mint` anchoring) | â€” | â€” | **NOT-DETECTED** | semantic field-level invariant; beyond pattern matching (documented limitation, see `EXPLOIT_CORPUS.md`) |
...
## Target 2 â€” Cashio, post-fix commit (`3f2c353`, 2022-04-23)

Same 5 findings (the vulnerable paths were never actually fixed â€” `brrr`'s
...
produces **zero findings** â€” properly-constrained Anchor code stays clean.

## Target 3 â€” Anchor tutorial examples (`coral-xyz/anchor` @ `474204e`, 2026-07-21)
...
| `basic-2` | 1Ã— SAT012 HIGH `+=` on u64 counter | **FP** for practical purposes â€” overflow needs 2^64 increments; valid lint-level note |
| `basic-4` | 1Ã— SAT012 HIGH `+=` (same as above) + 1Ã— SAT-`has_one` HIGH on `authority` | the `has_one` finding is an **FP** as exploit: the handler performs `require_keys_eq!(authority.key(), counter.authority)` â€” `sat` cannot see handler-level guards (FP pattern #2) |
...
- **Recall (root cause) on the one verified incident in scope: 0/1.**
  ...
  validation chain â€” a semantic invariant, explicitly outside `sat`'s detection
...
1. **FIXED â€” `CHECK:`-documented `UncheckedAccount` authorities were over-flagged.**
...
2. **Documented â€” handler-level key-equality guards are invisible to constraint
...
3. **Documented â€” `+=` on counters is lint-grade, not High.** Consider downgrading
...
(entrypoint/dispatch/account resolution) + 12 rules SAT019â€“SAT030. This is the
first run against real native programs â€” the recall/precision numbers below are
...
Command: `sat analyze src bench/programs/mango-v3/program/src` â†’ **16 findings**
(12 HIGH, 3 MEDIUM, 1 INFO).

| Finding | Count | Classification | Note |
|---|---|---|---|
| `Unverified Owner Account: token_account_ai` (SAT020) | 3 | **LIKELY leads** (manual confirm) | Verified in source: the findings are on the `RecoveryWithdrawTokenVault`/`MngoVault`/`InsuranceVault` handlers (processor.rs:8274+), which deserialize token data via `TokenAccount::load_checked` (zero-copy, no owner check â€” `Loadable::load` checks nothing). The subsequent `destination_account.owner == recovery_authority::ID` and mint-equality checks compare fields read from attacker-influenced account data â€” a self-referential validation shape (Cashio class). The pure CPI-pass-only shape (withdraw2's `token_account_ai`, passed only to `invoke_transfer`) is now suppressed by the CPI-passed-only fix |
| `Unsafe Arithmetic`/`Unsafe Multiplication` (SAT026) | 2 residual | **FP cluster FIXED; 2 residual lint-level** | The I80F48 checked-fixed-point cluster (`apply_fees`, `checked_add_net`, `checked_sub_net` â€” 12 findings) is now suppressed by the type-awareness fix. Two residual findings (`verify_bookside_iteration +=`, `cancel_all_advanced_orders +=`) are primitive u64 accumulators (loop counters/fee constants) â€” keyword-heuristic noise, not exploitable |
| INFO Token-2022 | 1 | informational | â€” |

**Read of the run:** 16 â†’ 6 findings after the FP fixes. Remaining: 3 genuine triage leads + 2 lint-level noise + INFO. Precision improved from ~35% to ~60% on unseen audited code without losing a single lead.
...
â†’ **0 findings**. The entrypoint engages (delegation chain to
`processor::process_instruction`), dispatch uses `LendingInstruction::unpack`.
Either genuinely clean (heavily audited) or below resolution depth â€” the
unpack-based dispatch path is a documented frontend limitation to verify next.

## Actions from this run (native)

1. **FIXED â€” FP pattern A (SAT020 on CPI-passed-only accounts):** `auth.rs`
...
2. **FIXED â€” FP pattern B (SAT026 type-blindness):** `lifecycle.rs` gained a
...
3. **FIXED â€” R4 guard gap:** `cpi.rs` SAT030 now counts discriminator/version/tag
   ...
   matching) â€” documented, future fix.
4. **Open â€” frontend depth:** `LendingInstruction::unpack`-style dispatch may
   resolve fewer instructions than real â€” add instruction-count rendering for
